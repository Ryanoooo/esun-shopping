...
LANGUAGE plpgsql AS $$
BEGIN
    INSERT INTO member(member_id, member_name, email, password_hash, phone)
    VALUES (p_member_id, p_member_name, p_email, p_password_hash, p_phone);
END;
$$;


-- SP0b: 依 Email 查詢會員（含 password_hash 和 role，用於登入驗證）
CREATE OR REPLACE FUNCTION sp_find_member_by_email(p_email VARCHAR)
RETURNS TABLE(
    member_id     VARCHAR,
    member_name   VARCHAR,
    email         VARCHAR,
...
BEGIN
    RETURN QUERY
        SELECT m.member_id, m.member_name, m.email, m.password_hash, m.phone,
               m.role,
