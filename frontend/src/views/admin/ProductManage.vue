  ...
  <div>
    ...
    <div class="card">
      <div class="section-header">
        <h2 class="section-title" style="margin:0">目前有庫存的商品</h2>
        <button class="btn btn-primary btn-sm" @click="loadProducts">重新整理</button>
      </div>
      ...
      <table v-else>
        <thead>
          <tr><th>商品編號</th><th>商品名稱</th><th>售價</th><th>庫存</th></tr>
        </thead>
        <tbody>
...
const handleAddProduct = async () => {
  ...
  try {
    ...
    errorMsg.value = e.response?.data?.message || '新增失敗，請確認商品編號是否重複'
  } finally {
    loading.value = false
  }
}
