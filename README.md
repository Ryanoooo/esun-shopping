...
簡易的電商購物中心平台，包含會員驗證、商品管理與訂單管理功能，採用三層式架構設計，並透過角色權限區分操作範圍。
...
> 腳本依數字順序執行，確保 DDL → SP → DML 的正確初始化順序。
...
#### JWT 驗證
- 登入後回傳 JWT Token，後續請求於 `Authorization: Bearer <token>` 回傳
- Token 包含 `memberId`、`role` 資訊，後端透過 `JwtAuthFilter` 解析並輸入 Spring Security Context
- Controller 使用 `@AuthenticationPrincipal` 直接取得登入者 ID，無需前端回傳
